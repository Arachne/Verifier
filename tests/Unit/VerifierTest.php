...
namespace Arachne\Tests\Verifier\Unit;

use Arachne\Tests\Verifier\Unit\Classes\TestControl;
use Arachne\Tests\Verifier\Unit\Classes\TestPresenter;
use Arachne\Tests\Verifier\Unit\Classes\TestRule;
...
class VerifierTest extends Test
{
	...
	public function testIsLinkVerifiedFalse()
	{
		...
		$handler = Mockery::mock('Arachne\Verifier\IRuleHandler')
			...
			->once()
			->with(Mockery::type('Arachne\Tests\Verifier\Unit\Classes\TestRule'), $request, NULL)
			->andThrow('Arachne\Tests\Verifier\Unit\Classes\TestException')
			->getMock();
	...
	private function createHandlerMock(Request $request, $limit, $component = NULL)
	{
		return Mockery::mock('Arachne\Verifier\IRuleHandler')
			->shouldReceive('checkRule')
			->times($limit)
			->with(Mockery::type('Arachne\Tests\Verifier\Unit\Classes\TestRule'), $request, $component)
			->andReturnNull()
			->getMock();
	}
	...
	private function setupHandlerLoaderMock(IRuleHandler $handler, $limit)
	{
		$this->handlerLoader
			...
			->with('Arachne\Tests\Verifier\Unit\Classes\TestRule')
			->times($limit)
			->andReturn($handler);
	}
	...
	private function setupPresenterFactoryMock()
	{
		$this->presenterFactory
			...
			->with('Test')
			->once()
			->andReturn('Arachne\Tests\Verifier\Unit\Classes\TestPresenter');
	}
